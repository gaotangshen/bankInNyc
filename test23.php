...
//var bonusArray= [];
var certArray = [];
function codeAddress(address,bankname,lat,lng,website,checkingLink,checkingName,summary,minBalance,monCharge,cert) {
	alert("111");
//	alert(cert);
			latArray[i]= lat;
			lngArray[i]=lng;
		...
		for(var k = j+1;k<count;k++){
			//alert(count);
				...
				if(distArray[j]>distArray[k]){
					...
					//	var temBonus = bonusArray[j];
					//	bonusArray[j] = bonusArray[k];
					//	bonusArray[k]=temBonus;
...
for(var x = 0;x<count;x++){
	...
	for(var y = 0; y<=count1-1;y++){
		alert(temCertArray[y]);
		alert(certArray[x]);
		alert(temCertArray[y]==certArray[x]);
...
//	alert(temArray[count1]);
	...
	//alert(endArray[x]);
	 ...
	 side_bar_html+='<a style=\"color: #191970; text-decoration: none; font-size: 17px;font-family: Calibri, Arial;\"'; 
		...
		side_bar_html+='<div id = "detail'+x+'show"; class = "hidden";><table><tr><td class="sidebar">Summary: </td><td class="sidebar">'+summaryArray[x]+ '</td></tr><tr><td class="sidebar">Minimum Balance: </td><td class="sidebar">'+minBanArray[x]+ '</td></tr><tr><td class="sidebar">Monthly Service Charge: </td><td class="sidebar">'+monChArray[x]+ '</td></tr><tr><td class="sidebar">Bonus: </td><td class="sidebar"></td></tr></table> </div>';
...
if(isset($_POST['zipcode'])){
	...
	from branch,personalcheking as p where zip >('$_POST[zipcode]'-10) and zip < ('$_POST[zipcode]'+10)
	and branch.name = p.bankname and branch.cert = p.cert and FreeOrNot = 1 limit 2" ;
	...
	while($row = mysql_fetch_array($query)){
		...
		$monServiceCharge.=$row['monthlyServiceCharge'].'@#'.' ';
		//$bonus.=$row['bonus'].'@#&'.' ';
	...
	<script type="text/javascript">
		fu = null;
		function showAdd(addr,bankname,lat,lng,website,checkingLink,checkingName,summary,minimumBalance,monServiceCharge,cert){
			alert("12313");
			clearInterval(fu);
			...
			codeAddress(loc[i],bname[i],lat[i],lng[i],web[i],checkingLink[i],checkingName[i],summary[i],minBalance[i],monCharge[i],cert[i]);
		...
		fu = setInterval(showAdd, 1000,"<?=$combine?>","<?=$name?>","<?=$lat?>","<?=$lng?>","<?=$website?>","<?=$personalcheckingLink?>","<?=$checkingName?>","<?=$summary?>","<?=$minimumBalance?>","<?=$monServiceCharge?>","<?=$cert?>");
